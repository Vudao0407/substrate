...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
		pub fn map_num_to_account (caller: OriginFor<T>,number: u32) -> DispatchResult {
			let who = ensure_signed(caller)?;
			<SomeMap<T>>::set(&who, number);
			// Emit an event.
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
		pub fn remove_number (caller: OriginFor<T>) -> DispatchResult {
			let who = ensure_signed(caller)?;
			<SomeMap<T>>::remove(&who);
			Ok(())
		}

		/// getNumber
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
		pub fn get_number (caller: OriginFor<T>) -> DispatchResult {
			let who = ensure_signed(caller)?;
			<SomeMap<T>>::get(&who);
			Ok(())
		}
